...
from clarity.Settings import clarityPath

DefaultLabeledImageFile = os.path.join(clarityPath, r'Data\annotation_25_half_sagittal.mhd')
DefaultAnnotationFile   = os.path.join(clarityPath, r'Data\annotation_info_collapse.csv')
...
def makeColorAnnotationsSelectLabelAtLevel(filename, label, level, excludedLabels = [], labeledImage = None):
    if labeledImage is None:
        labeledImage = DefaultLabeledImageFile
    ...
    colarray = numpy.zeros((maxlabel, 3))
    for i in Label.ids:
        if labelAtLevel(i,level) == label and i not in excludedLabels:
            colarray[i-1,:] = Label.color(i)
        else:
            colarray[i-1,:] = [0,0,0]
...
def makeColorAnnotationsSelectLabelAtLevel2(filename, label, excludedLabels = [], labeledImage = None):
	# Different from the previous function: this function does not use labelAtLevel: ignores the level to make color annotation. 
	if labeledImage is None:
		labeledImage = DefaultLabeledImageFile
	...
	colarray = numpy.zeros((maxlabel, 3))
	for i in Label.ids:
		if i in label and i not in excludedLabels:
			colarray[i-1,:] = Label.color(i)
		else:
			colarray[i-1,:] = [0,0,0]
